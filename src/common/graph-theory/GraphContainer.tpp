...
public:
  ...
  bool marked(const size_t nth) const
  {
    ...
    return 0 != (Collection<T, N, Block>::m_blocks[index]->m_marked[subindex / S] & (1u << (MODULO(subindex, S))));
  }
  ...
  void mark(const size_t nth)
  {
    ...
    Collection<T, N, Block>::m_blocks[index]->m_marked[subindex / S] |= (1u << (MODULO(subindex, S)));
  }
