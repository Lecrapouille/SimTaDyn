...
public:
  ...
  {
    LOGD("Prog '%s' attaching Shader '%s'", cname(), shader.cname());
  ...
  {
    LOGD("Prog '%s' attaching Shaders '%s, %s, %s'", cname(),
  ...
  GLProgram& attachShaders(GLVertexShader&   vertex_shader,
                           GLFragmentShader& fragment_shader)
  {
    LOGD("Prog '%s' attaching Shaders '%s, %s'", cname(),
  ...
  {
    LOGD("Binding VAO '%s' to Prog '%s'", vao.cname(), cname());
    ...
    if (unlikely(!isCompiled()))
      {
        if (!compile())
          {
            LOGE("Tried to bind VAO '%s' on a non compilable GLProgram '%s'",
                  vao.cname(), cname());
            return false;
          }
      }
    ...
    if (unlikely(0 == vao.prog))
      {
        ...
        LOGD("%s", "  First binding. VBOs will be populated {");
        initVAO(vao);
        LOGD("%s", "  } First binding. VBOs will be populated");
      }
    else if (unlikely(m_handle != vao.prog))
      {
        ...
        LOGE("Tried to bind VAO '%s' already bound to another Prog than '%s'",
             vao.cname(), cname());
        return false;
      }
  ...
  {
    LOGD("Prog '%s' get uniform '%s'", cname(), name);
  ...
  {
    LOGD("Prog '%s' const get uniform '%s'", cname(), name);
    return getUniform<T>(name).data();
  }
  ...
  {
    LOGD("Prog '%s' draw VAO '%s' {", cname(), vao.cname());
  ...
  {
    LOGD("Prog '%s' draw %zu elements {", cname(), m_vao->m_vbos.begin()->second->size());
    throw_if_not_compiled();
    throw_if_vao_not_bound();
    throw_if_inconsitency_attrib_sizes();
  ...
  {
    LOGD("Prog '%s' drawIndex %zu elements {", cname(), index.size());

    throw_if_not_compiled();
    throw_if_vao_not_bound();
    throw_if_inconsitency_attrib_sizes();

    LOGD("Prog '%s' begin {", cname());
    begin();
    LOGD("} Prog '%s' begin", cname());

    LOGD("Prog '%s' index begin {", cname());
    index.begin();
    LOGD("} Prog '%s' index begin", cname());

    LOGD("%s", "Draw Elements");
    glCheck(glDrawElements(static_cast<GLenum>(mode),
                           static_cast<GLsizei>(index.size()),
                           index.type(), 0));
    LOGD("Prog '%s' end index, VAO", cname());
    index.end();
    glCheck(glBindBuffer(GL_ARRAY_BUFFER, 0));
    end();
    LOGD("} Prog '%s' drawIndex ", cname());
  }
...
private:
  ...
  {
    LOGD("<<<<<<<<<<<< Prog '%s' draw VAO '%s' {", cname(), m_vao->cname());
    ...
    LOGD("Draw Arrays %u %zu %zu", static_cast<uint32_t>(mode), first, count);
    glCheck(glDrawArrays(static_cast<GLenum>(mode),
                         static_cast<GLint>(first),
                         static_cast<GLsizei>(count)));

    LOGD("Prog '%s' end VAO", cname());
    glCheck(glBindBuffer(GL_ARRAY_BUFFER, 0));
    end();
    LOGD(">>>>>>>>>>>> } Prog '%s'", cname());
  }
  ...
  /*inline virtual bool isValid() const override
    {
    LOGD("Prog::isValid %d", isCompiled());
    return isCompiled();
    }*/
    ...
    for (auto& it: m_samplers)
      {
        ...
        switch (gltype)
          ...
          case GL_SAMPLER_CUBE:
            ...
            break;
          default:
            LOGE("This kind of sampler is not yet managed: %u", gltype);
            break;
          }
      }
  ...
  virtual void activate() override
  {
    ...
    LOGD("Prog '%s' -> VAO '%s' begin {", cname(), m_vao->cname());
    m_vao->begin();
    LOGD("} Prog '%s' -> VAO '%s' begin", cname(), m_vao->cname());

    for (auto& it: m_attributes)
      {
        LOGD("Prog '%s' -> VAO '%s' -> VBO '%s' begin {",
              cname(), m_vao->cname(), it.first.c_str());
        m_vao->m_vbos[it.first]->begin();
        LOGD("} Prog '%s' -> VAO '%s' -> VBO '%s' begin",
              cname(), m_vao->cname(), it.first.c_str());

        LOGD("Prog '%s' -> VAO '%s' -> attribute '%s' begin {",
    ...
    for (auto& it: m_uniforms)
      {
        LOGD("Prog '%s' -> uniform '%s' begin {", cname(), it.second->cname());
        it.second->begin();
        LOGD("} Prog '%s' -> uniform '%s' begin", cname(), it.second->cname());
      }
    for (auto& it: m_samplers)
      {
        ...
        // texture.
        LOGD("Prog '%s' -> sampler '%s' begin {", cname(), it.second->cname());
        it.second->begin();
        LOGD("} Prog '%s' -> sample '%s' begin", cname(), it.second->cname());
  ...
  {
    LOGD("Prog '%s' setup {", cname());
    bool failure = false;

    // Compile shaders if they have not yet compiled
    LOGD("Prog '%s' compile shaders {", cname());
    ...
    LOGD("} Prog '%s' compile shaders. Failure? %u", cname(), failure);
    ...
    if (!failure)
      {
        ...
        LOGD("Prog '%s' link shaders", cname());
    ...
    LOGD("} Prog '%s' setup. Linked? %u", cname(), m_compiled);
    return !m_compiled;
  }
  ...
  void createAllLists()
  {
    ...
    LOGD("Prog '%s' populating shader uniforms and samplers {", cname());
    ...
    LOGD("Prog '%s' populating shader attributes {", cname());
    ...
    if (GL_FALSE == status)
      {
        ...
        LOGE("Failed linking '%s'. Reason was '%s'", cname(), m_error_msg.c_str());
      }
    else
      {
        m_error_msg.clear();
