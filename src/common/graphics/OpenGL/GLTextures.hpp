                     ...
                     TextureData& data, uint32_t& width, uint32_t& height)
{
  ...
  LOGD("Loading texture '%s'", filename);
  ...
  if (likely(nullptr != image))
    {
      ...
      LOGD("Successfully loaded %ux%u texture '%s'", width, height, filename);
      return true;
    }
  else
    {
      LOGE("Failed loading picture file '%s'", filename);
      width = height = 0;
      data.clear();
      return false;
    }
...
public:
 ...
 GLTexture2D(std::string const& name, const uint32_t width, const uint32_t height)
  ...
  {
    LOGD("Texture2D '%s' load bitmap '%s'", cname(), filename);
  ...
  {
    LOGD("Texture '%s' setup", cname());

    // Note: m_buffer can nullptr
    if (unlikely(!loaded()))
      {
        LOGE("Cannot setup texture '%s'. Reason 'Data not yet loaded'", cname());
        return true;
      }
...
#  pragma GCC diagnostic ignored "-Wconversion"
    ...
    LOGD("Texture '%s' update (%zu,%zu) --> ((%d,%d), (%d,%d))",
...
public:
  ...
  bool load(CubeMap const target, const char *const filename)
  {
    ...
    LOGD("TextureCube '%s' %zu load bitmap '%s'", cname(), index, filename);
...
public:
  ...
  bool load(std::vector<std::string> const& filenames)
  {
    ...
    for (size_t i = 0; i < depth; ++i)
      {
        LOGD("Texture3D '%s' load bitmap '%s'", cname(), filenames[i].c_str());
        ...
        if ((i != 0) && ((prevWidth != width) || (prevHeight != height)))
          {
            LOGE("Failed picture file '%s' has not correct dimension %ux%u",
